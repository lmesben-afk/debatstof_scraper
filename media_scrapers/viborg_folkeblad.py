"""
Viborg Folkeblad-specific scraper.
...
Formål:
- Brug kun https://viborg-folkeblad.dk/debat
...
def viborg_debate_type_from_text(text: str, helpers) -> str:
...
def viborg_url_looks_like_debate_article(url: str) -> bool:
    value = (url or "").lower()

    if "viborg-folkeblad.dk" not in value:
        return False
    ...
    if any(part in value for part in bviborg_parts):
        return False

    return True


def clean_viborg_title(title: str, helpers) -> str:
...
def viborg_metadata_looks_wrong(text: str, helpers) -> bool:
...
def viborg_author_looks_wrong(author: str, helpers) -> bool:
    value = helpers["normalize_label_text"](author or "")

    wrong_fragments = [
        "henrik lund",
        "helu@viborg-folkeblad.dk",
    ]
...
def viborg_text_is_bad_deck(text: str, helpers) -> bool:
    value = helpers["normalize_label_text"](text or "")
    if not value:
        return True
    bad_fragments = [
...
def viborg_extract_deck_from_article_page(client, url: str, title: str, helpers) -> str:
    try:
    ...
    for candidate in candidates:
        ...
        if title_norm and (value == title_norm or value in title_norm):
            continue
        if viborg_text_is_bad_deck(text, helpers):
            continue
...
def viborg_extract_article_page_fields(client, url: str, title: str, helpers) -> dict:
    """
    Henter Viborg Folkeblad-felter direkte fra artikelsiden.
    ...
    for author in author_candidates:
        ...
        if "@" in author and len(author) < 80:
            continue
        if "henrik lund" in value or "helu@viborg-folkeblad.dk" in value:
            continue
...
def viborg_title_is_navigation(title: str, helpers) -> bool:
...
def viborg_find_marker_near_link(a, helpers) -> str:
    """
    Finder stofmærket tæt på rubrikken.

    Viborg Folkeblad viser markøren lige over rubrikken, men den er typisk
    ...
    for container in containers:
        ...
        for el in container.find_all(["span", "div", "p", "strong", "em"], recursive=True):
            text = helpers["clean_text"](el.get_text(" ", strip=True))
            value = helpers["normalize_label_text"](text)
            if value in VIBORG_MARKER_WORDS:
                return text
    ...
    for container in containers:
        ...
        dtype = viborg_debate_type_from_text(before_tail, helpers)
        if dtype:
            return before_tail
...
def scrape_viborg_direct(client, source, show_urls: bool = False, limit: Optional[int] = None, helpers=None) -> List:
    if helpers is None:
        raise RuntimeError("Viborg Folkeblad scraper mangler helpers")
    ...
    section_url = "https://viborg-folkeblad.dk/debat"
    html_text = helpers["fetch_text"](client, section_url)

    if not html_text:
        print("[VIBORG DEBUG] Kunne ikke hente debatforsiden.")
        return items

    soup = BeautifulSoup(html_text, "lxml")
    all_links = soup.find_all("a", href=True)

    print(f"[VIBORG DEBUG] Hentede {len(html_text)} tegn fra {section_url}")
    print(f"[VIBORG DEBUG] Fandt {len(all_links)} links i HTML.")
    ...
    for a in all_links:
        ...
        if not viborg_url_looks_like_debate_article(absolute):
            continue

        if absolute in seen:
            continue

        marker_text = viborg_find_marker_near_link(a, helpers)
        debate_type = viborg_debate_type_from_text(marker_text, helpers)
        ...
        title = clean_viborg_title(link_text, helpers)

        if not title or len(title) < 8:
            continue

        if viborg_title_is_navigation(title, helpers):
            continue

        article_fields = viborg_extract_article_page_fields(client, absolute, title, helpers)
    ...
    print(f"[VIBORG DEBUG] Accepterede {len(items)} Viborg Folkeblad-artikler fra debatforside.")

    if not items:
        print("[VIBORG DEBUG] Ingen artikler accepteret. Første 100 relevante links/kort:")
        for i, d in enumerate(diagnostics[:100], start=1):
            print(f"[VIBORG DEBUG] {i}.")
            print(f"[VIBORG DEBUG]    debat_type={d['debate_type'] or '-'}")
            print(f"[VIBORG DEBUG]    marker={d.get('marker') or '-'}")
            print(f"[VIBORG DEBUG]    linktekst={d['link_text'][:220] or '-'}")
            print(f"[VIBORG DEBUG]    url={d['url']}")
