"""
JydskeVestkysten-specific scraper.
...
def jv_debate_type_from_text(text: str, helpers) -> str:
    ...
    # JV bruger nogle gange geografisk markør over rubrikken i stedet for debat-type.
    # Hvis linket ligger på /debat/ og markøren er et lokalområde, beholder vi artiklen
    # og standardiserer typen til Debat.
    if value in JV_PLACE_MARKERS:
        return "Debat"

    return ""


def jv_url_looks_like_debate_article(url: str) -> bool:
    value = (url or "").lower()

    if "jv.dk" not in value:
        return False
    ...
    if any(part in value for part in bjv_parts):
        return False

    return True


def clean_jv_title(title: str, helpers) -> str:
...
def jv_metadata_looks_wrong(text: str, helpers) -> bool:
...
def jv_author_looks_wrong(author: str, helpers) -> bool:
...
def jv_text_is_bad_deck(text: str, helpers) -> bool:
    value = helpers["normalize_label_text"](text or "")
    if not value:
        return True
    bad_fragments = [
...
def jv_extract_deck_from_article_page(client, url: str, title: str, helpers) -> str:
    try:
    ...
    for candidate in candidates:
        ...
        if title_norm and (value == title_norm or value in title_norm):
            continue
        if jv_text_is_bad_deck(text, helpers):
            continue
...
def jv_extract_article_page_fields(client, url: str, title: str, helpers) -> dict:
    """
    Henter JydskeVestkysten-felter direkte fra artikelsiden.
    ...
    for author in author_candidates:
        ...
        if "@" in author and len(author) < 80:
            continue
        if "henrik lund" in value or "helu@jv.dk" in value:
            continue
...
def jv_title_is_navigation(title: str, helpers) -> bool:
...
def jv_find_marker_near_link(a, helpers) -> str:
    """
    Finder stofmærket tæt på rubrikken.

    JydskeVestkysten viser markøren lige over rubrikken, men den er typisk
    ...
    for container in containers:
        ...
        for el in container.find_all(["span", "div", "p", "strong", "em"], recursive=True):
            text = helpers["clean_text"](el.get_text(" ", strip=True))
            value = helpers["normalize_label_text"](text)
            if value in JV_MARKER_WORDS or value in JV_PLACE_MARKERS:
                return text
    ...
    for container in containers:
        ...
        dtype = jv_debate_type_from_text(before_tail, helpers)
        if dtype:
            return before_tail
...
def scrape_jv_direct(client, source, show_urls: bool = False, limit: Optional[int] = None, helpers=None) -> List:
    if helpers is None:
        raise RuntimeError("JydskeVestkysten scraper mangler helpers")
    ...
    section_url = "https://jv.dk/debat"
    html_text = helpers["fetch_text"](client, section_url)

    if not html_text:
        print("[JV DEBUG] Kunne ikke hente debatforsiden.")
        return items

    soup = BeautifulSoup(html_text, "lxml")
    all_links = soup.find_all("a", href=True)

    print(f"[JV DEBUG] Hentede {len(html_text)} tegn fra {section_url}")
    print(f"[JV DEBUG] Fandt {len(all_links)} links i HTML.")
    ...
    for a in all_links:
        ...
        if not jv_url_looks_like_debate_article(absolute):
            continue

        if absolute in seen:
            continue

        marker_text = jv_find_marker_near_link(a, helpers)
        debate_type = jv_debate_type_from_text(marker_text, helpers)
        ...
        title = clean_jv_title(link_text, helpers)

        if not title or len(title) < 8:
            continue

        if jv_title_is_navigation(title, helpers):
            continue

        article_fields = jv_extract_article_page_fields(client, absolute, title, helpers)
    ...
    print(f"[JV DEBUG] Accepterede {len(items)} JydskeVestkysten-artikler fra debatforside.")

    if not items:
        print("[JV DEBUG] Ingen artikler accepteret. Første 100 relevante links/kort:")
        for i, d in enumerate(diagnostics[:100], start=1):
            print(f"[JV DEBUG] {i}.")
            print(f"[JV DEBUG]    debat_type={d['debate_type'] or '-'}")
            print(f"[JV DEBUG]    marker={d.get('marker') or '-'}")
            print(f"[JV DEBUG]    linktekst={d['link_text'][:220] or '-'}")
            print(f"[JV DEBUG]    url={d['url']}")
