"""
Avisen Danmark-specific scraper.
...
Formål:
- Brug kun https://avisendanmark.dk/debat
...
def ad_debate_type_from_text(text: str, helpers) -> str:
...
def ad_url_looks_like_debate_article(url: str) -> bool:
    value = (url or "").lower()

    if "avisendanmark.dk" not in value:
        return False
...
def clean_ad_title(title: str, helpers) -> str:
...
def ad_title_is_navigation(title: str, helpers) -> bool:
...
def ad_find_marker_near_link(a, helpers) -> str:
    """
    Finder stofmærket tæt på rubrikken.

    Avisen Danmark viser markøren lige over rubrikken, men den er typisk
    ...
    for container in containers:
        ...
        dtype = ad_debate_type_from_text(before_tail, helpers)
        if dtype:
            return before_tail
...
def ad_text_is_bad_deck(text: str, helpers) -> bool:
...
def ad_extract_article_page_fields(client, url: str, helpers) -> dict:
    result = {
        "deck": "",
        "author": "",
        "published_at": "",
    ...
    # Manchet: kun billedtekst, samme princip som Stiften/Fyens.
    caption_candidates = []
    ...
    for caption in caption_candidates:
        ...
        if len(text) < 25 or len(text) > 650:
            continue
        if ad_text_is_bad_deck(text, helpers):
            continue
...
def scrape_avisen_danmark_direct(client, source, show_urls: bool = False, limit: Optional[int] = None, helpers=None) -> List:
    if helpers is None:
        raise RuntimeError("Avisen Danmark scraper mangler helpers")
    ...
    section_url = "https://avisendanmark.dk/debat"
    html_text = helpers["fetch_text"](client, section_url)

    if not html_text:
        print("[AD DEBUG] Kunne ikke hente debatforsiden.")
        return items

    soup = BeautifulSoup(html_text, "lxml")
    all_links = soup.find_all("a", href=True)

    print(f"[AD DEBUG] Hentede {len(html_text)} tegn fra {section_url}")
    print(f"[AD DEBUG] Fandt {len(all_links)} links i HTML.")
    ...
    for a in all_links:
        ...
        if not ad_url_looks_like_debate_article(absolute):
            continue

        if absolute in seen:
            continue

        marker_text = ad_find_marker_near_link(a, helpers)
        debate_type = ad_debate_type_from_text(marker_text, helpers)
        ...
        title = clean_ad_title(link_text, helpers)

        if not title or len(title) < 8:
            continue

        if ad_title_is_navigation(title, helpers):
            continue

        article_fields = ad_extract_article_page_fields(client, absolute, helpers)
    ...
    print(f"[AD DEBUG] Accepterede {len(items)} Avisen Danmark-artikler fra debatforside.")

    if not items:
        print("[AD DEBUG] Ingen artikler accepteret. Første 100 relevante links/kort:")
        for i, d in enumerate(diagnostics[:100], start=1):
            print(f"[AD DEBUG] {i}.")
            print(f"[AD DEBUG]    debat_type={d['debate_type'] or '-'}")
            print(f"[AD DEBUG]    marker={d.get('marker') or '-'}")
            print(f"[AD DEBUG]    linktekst={d['link_text'][:220] or '-'}")
            print(f"[AD DEBUG]    url={d['url']}")
